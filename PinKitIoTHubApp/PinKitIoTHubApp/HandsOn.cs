...
{
    public partial class Program //部分クラス
    {
        ...
        {
            //マクロ(ACCESS_MOBILE_SERVICE,ACCESS_IOT_HUB)に応じてIoTServiceAvailabledをTRUEにする。
            //sensorReadingsオブジェクトを生成
            EntryDevice();
            if (IoTServiceAvailabled)
            {
                SetupIoTHub();//IotHubのセットアップ
            }
        }
        ...
        {
            InitializeUpload(); //IotHubへのアップロードの初期化
